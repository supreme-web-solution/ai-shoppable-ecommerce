...
function accountKey(account: ZernioAccount): string {
...
async function loadAccountsAndShop() {
    ...
    try {
        ...
        const [status, shop] = await Promise.all([
            apiFetch<{ accounts?: ZernioAccount[] }>(`/api/v1/admin/zernio/status?team_id=${id}`),
...
async function publish() {
    ...
    try {
        ...
        successText.value = 'Post queued with Zernio. Your shop link is included in the caption.';
        emit('published');
    } catch (err) {
    ...
    <div class="space-y-3 rounded-xl border border-[#E8563A]/20 bg-[#E8563A]/5 p-3">
        <div>
            ...
            <p class="mt-0.5 text-[11px] text-gray-600">
                Video is posted natively; caption includes your mobile shop page (products + checkout).
            </p>
        </div>
